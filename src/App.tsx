...
function Reader({ book, onBack, onSelect, onProgress }: { book?: Book; onBack: () => void; onSelect: (word: WordDefinition) => void; onProgress: (progress: number) => void }) {
  ...
  const changePage = (nextPage: number) => {
    ...
    setPage(boundedPage)
    onProgress((boundedPage + 1) / pageCount)
  }
  useEffect(() => { setPage(0) }, [book?.id])
  useEffect(() => {
    const onKeyDown = (event: KeyboardEvent) => {
      if (event.key === 'ArrowRight' || (event.key === ' ' && !event.shiftKey)) { event.preventDefault(); changePage(page + 1) }
      if (event.key === 'ArrowLeft' || (event.key === ' ' && event.shiftKey)) { event.preventDefault(); changePage(page - 1) }
    }
    window.addEventListener('keydown', onKeyDown)
    return () => window.removeEventListener('keydown', onKeyDown)
  }, [page, pageCount])
  if (!book) return <EmptyLibrary onImport={onBack} />
  const visibleWords = words.slice(page * wordsPerPage, (page + 1) * wordsPerPage)
  return <div className="reader"><div className="reader-bar"><button className="quiet" onClick={onBack}>← Library</button><span>{book.title}</span><div><button className="quiet" onClick={() => setFont(Math.max(16, font - 1))}>A−</button><button className="quiet" onClick={() => setFont(Math.min(28, font + 1))}>A+</button><button className="quiet" onClick={() => onProgress((page + 1) / pageCount)}>Bookmark</button></div></div><article className="reading" style={{ fontSize: font }}>{visibleWords.map((token, index) => /[a-zA-Z]/.test(token) ? <button key={index} className="reading-word" onClick={() => onSelect(dictionaryService.lookup(token))}>{token}</button> : token)}<div className="reader-end"><div className="progress"><i style={{ width: `${(page + 1) / pageCount * 100}%` }} /></div><span>Page {page + 1} of {pageCount} · {Math.round((page + 1) / pageCount * 100)}% complete</span><div className="page-actions"><button className="quiet" disabled={page === 0} onClick={() => changePage(page - 1)}>← Previous</button><button className="primary" disabled={page === pageCount - 1} onClick={() => changePage(page + 1)}>Next →</button></div></div></article></div>
}
